...
scaffold unrelated application source folders, platform shells, package scripts,
CI, or tests unless a later story explicitly moves that area into
implementation.
...
This harness does not ship with a project-specific `SPEC.md`. LLM-Local product
docs were derived from the current repository and spec intake notes. When the
human provides a new spec or initiative, treat that input as material for the
next buildout. Product docs, stories, tests, and decisions are the living
contract that agents should update as the system evolves.
